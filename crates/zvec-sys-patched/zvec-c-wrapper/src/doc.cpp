...
bool zvec_doc_get_string(const zvec_doc_t* doc, const char* field, const char** out_value) {
    if (doc && doc->ptr && field && out_value) {
        ...
        if (result.has_value()) {
            // Cache the string in the doc struct so the returned pointer
            // remains valid (result is a local copy that would otherwise
            // be destroyed before the caller reads *out_value).
            doc->cached_string_value = std::move(result.value());
            *out_value = doc->cached_string_value.c_str();
            return true;
        }
    }
    return false;
