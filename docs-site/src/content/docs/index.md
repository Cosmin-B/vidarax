...
write-ahead log. Selected JPEGs and retained MP4 windows live in
content-addressed binary stores while events carry references. Media bytes
never enter JSON, SSE, webhooks, or the WAL. Consumers can use the TypeScript
SDK, the Vue 3 interface, cursor-based SSE, signed webhooks, or the REST API.
...
backend, and storage. The event store retains metadata, selected JPEGs, and MP4
windows when requested. It never retains the full source automatically.
