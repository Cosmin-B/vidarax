...
Reclaim is race-safe by construction. `remove_session_for_run` uses `DashMap::remove_if` to check run ownership and remove under one shard lock. Exactly one caller wins cleanup, then writes a `ReclaimedSessions` record so a later DELETE of the same session stays idempotent and avoids a 404. A ten-minute TTL and a 1,024-entry cap bound that tombstone map.
...
- `synthetic_run_deleted_event` stamps the snapshot's current `max_seq` and allocates no new sequence number. It exists only to shape an HTTP response and is never persisted.
